...
/**
 * \ingroup randomvariable
 * \brief The burr distribution Random Number Generator (RNG) 
 *
 *
 * Zeta distribution has one parameter, alpha, \f$ \alpha > 1 \f$ (real).
 * Probability Mass Function is \f$ f(k; \alpha) = k^{-\alpha}/\zeta(\alpha) \f$
 ...
 * \endcode
 */
class BurrRandomVariable : public RandomVariableStream
{
public:
  /**
   * \brief Register this type.
  ...
  /**
   * \brief Creates a burr distribution RNG with the default value for
   * alpha.
   */
  BurrRandomVariable ();
   ...
   * which is the distance \f$u\f$ is from the 1.
   */
  double GetValue (double landa, double c, double k);
   ...
   * which is the distance \f$u\f$ is from the 1.
   */
  uint32_t GetInteger (uint32_t landa, uint32_t c, uint32_t k);
...
private:
  /** The alpha value for the zeta distribution returned by this RNG stream. */
  double m_landa, m_c, m_k;

};  // class ZetaRandomVariable

class ExtremeRandomVariable : public RandomVariableStream
{
public:
  /**
   * \brief Register this type.
   ...
   * which is the distance \f$u\f$ is from the 1.
   */
  double GetValue (double loc, double scale);
   ...
   * which is the distance \f$u\f$ is from the 1.
   */
  uint32_t GetInteger (uint32_t loc, uint32_t scale);
...
private:
  /** The alpha value for the zeta distribution returned by this RNG stream. */
  double m_loc, m_scale;

};  // class ExtremeRandomVariable


class tLocRandomVariable : public RandomVariableStream
{
public:
  /**
   * \brief Register this type.
   ...
   * which is the distance \f$u\f$ is from the 1.
   */
  double GetValue (double v, double loc, double scale);
   ...
   * which is the distance \f$u\f$ is from the 1.
   */
  uint32_t GetInteger (uint32_t v,uint32_t loc, uint32_t scale);
...
private:
  /** The alpha value for the zeta distribution returned by this RNG stream. */
  double m_v, m_loc, m_scale;
  double gamma (double n);
  double Vcalc (double u, double n);
  double coef (uint8_t i, double v);
